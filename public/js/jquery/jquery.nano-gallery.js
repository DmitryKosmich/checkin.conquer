/**!
 * @preserve nanoGALLERY v4.4.3beta
...
/*
 ...
 nanoGALLERY v4.4.3beta release notes.


 ##### New features:
 - improved image display on touch display
 - new javascript helpers (fnViewerInfo, fnProcessData)
 ...
 - parameter touchAnimation automatically set to false depending on the selected hover effects

 ##### New options:
 - fnViewerInfo: javascript helper for info button on viewer toolbar
 - fnProcessData: javascript helper to extend data associated to thumbnails/images
 ...
 **Visit nanoGALLERY homepage for usage details: [http://nanogallery.brisbois.fr](http://www.nanogallery.brisbois.fr/)**
 ...
 - fixed bug in themes clean and light
...
(function ($) {
    jQuery.fn.nanoGallery = function (options) {
        var settings = $.extend(true, {
            ...
            fnViewerInfo : null,
            fnProcessData : null,   //function(newItem, kind, type, data) { alert('hello'); newItem.title='pipo'; },  //pipo
            touchAnimation : true,
            useTags : false,
            preset : 'none',
            locationHash : false,
...
function nanoGALLERY() {
    ...
    var g_i18nTranslations = {'paginationPrevious':'Previous', 'paginationNext':'Next', 'breadcrumbHome':'List of Albums', 'thumbnailImageTitle':'', 'thumbnailAlbumTitle':'', 'thumbnailImageDescription':'', 'thumbnailAlbumDescription':'' },
        ...
        g_viewerSwipe = null,
        g_aengine='animate',
        g_transformProperty=null;

    g_transformProperty=getSupportedPropertyName();
...
//      }
    ...
    function checkPluginParameters() {
            ...
            case 'string':
                ...
                for(var i=0; i<tmp.length; i++) {
                    if( isAEngineSupported(tmp[i]) ) {
                        ...
                        oDef.name=tmp[i];
                        g_thumbnailHoverEffect.push(oDef);
                    }
                }
                break;
            case 'object':
                if( isAEngineSupported(gO.thumbnailHoverEffect.name) ) {
                    var oDef=NewTHoverEffect();
                    g_thumbnailHoverEffect.push(jQuery.extend(oDef,gO.thumbnailHoverEffect));
                }
                break;
            case 'array':
                for(var i=0; i<gO.thumbnailHoverEffect.length; i++) {
                    if( isAEngineSupported(gO.thumbnailHoverEffect[i].name) ) {
                        var oDef=NewTHoverEffect();
                        g_thumbnailHoverEffect.push(jQuery.extend(oDef,gO.thumbnailHoverEffect[i]));
                    }
                }
                break;
            case 'null':
        ...
        for( var i=0; i<g_thumbnailHoverEffect.length; i++) {
            if( !/labelOpacity50|borderLighter|borderDarker/i.test(g_thumbnailHoverEffect[i].name) ) { touchAnim=true; }
        }

        if( !(typeof gO.fnThumbnailHoverInit == 'function') && !touchAnim ) {
    ...
    function NewTHoverEffect() {
        ...
        var oDef={'duration':200,'durationBack':150,'easing':'swing','easingBack':'swing'};
    ...
    function isAEngineSupported( effect ) {
        ...
        var isStd = /slideUp|slideDown|slideLeft|slideRight|imageSlideUp|imageSlideDown|imageSlideLeft|imageSlideRight|labelAppear|labelAppear75|labelSlideDown|labelSlideUp|labelSlideUpTop|imageInvisible|imageOpacity50|descriptionSlideUp|labelSplitVert|labelSplit4|labelAppearSplitVert|labelAppearSplit4|imageSplitVert|imageSplit4/i.test(effect);
        var isAdv = /imageScale150|imageScale150Outside|scale120|overScale|overScaleOutside|scaleLabelOverImage|imageFlipHorizontal|imageFlipVertical|rotateCornerBR|rotateCornerBL|imageRotateCornerBR|imageRotateCornerBL/i.test(effect);
        ...
        if( gO.thumbnailLabel.position == 'onBottom' && !/borderLighter|borderDarker|imageOpacity50|imageScale150|imageScale150Outside|scale120/i.test(effect) ) {
        ...
        if( (isAdv && (g_aengine == 'animate' || g_transformProperty == null) ) ) {
            nanoConsoleLog('Parameter thumbnailHoverEffect="'+effect+'" requires the additional jQuery plugin "Velocity" or "Transit".');
            return false;
        }
    ...
    function retrieveThumbnailSizes() {
        ...
        var item=NGAddItem('dummy', 'data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=', 'data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=', '', '', 'image', '', '1', '0' );
    ...
    function ProcessItemOption() {
        ...
        jQuery.each(gO.items, function(i,item){
            ...
            if( typeof gO.fnProcessData == 'function' ) {
                gO.fnProcessData(newItem, 'api', kind, null);
            }
    ...
    function ProcessHREF(elements) {
        ...
        jQuery.each(elements, function(i,item){
            ...
            if( typeof gO.fnProcessData == 'function' ) {
                gO.fnProcessData(newItem, 'markup', kind, null);
            }
        ...
        jQuery.each(data.feed.entry, function(i,data){
            ...
            if( ok ) {
                ...
                if( g_thumbScale > 1 ) {
                    ...
                    itemThumbURL=s+'s'+(Math.max(gO.thumbnailWidth,gO.thumbnailHeight)*g_thumbScale)+'/';
                }
                ...
                if( typeof gO.fnProcessData == 'function' ) {
                    gO.fnProcessData(newItem, 'picasa', kind, data);
                }
    ...
    function SmugmugParseData( albumIdx, data, kind ) {
        ...
        if( kind == 'album' ) {
            ...
            jQuery.each(data.Albums, function(i,data){
                ...
                //var imgUrl='//:0';
                var imgUrl='data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=';
                ...
                if( ok ) {
                    ...
                    if( typeof gO.fnProcessData == 'function' ) {
                        gO.fnProcessData(newItem, 'smugmug', kind, data);
                    }
...
//    if( g_containerThumbnailsDisplayed ) {
        ...
        jQuery($eltInViewport).each(function(){
            ...
            if( jQuery($image).attr('src') == "data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=" ) {
...
//    }
    ...
    function thumbnailBuild( item, idx, eltOpacity, foundDesc ) {
        ...
        if( checkImageSize ) {
            $newDiv.ngimagesLoaded().always( function( instance ) {
                ...
                if( item == undefined || jQuery(instance.images[0].img).attr('src') == 'data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=' ) { return; }    // also fired for blank image --> ignore
                var b=false;
    ...
    function ThumbnailInit( elt ) {
        ...
        for( j=0; j<g_thumbnailHoverEffect.length; j++) {
            switch(g_thumbnailHoverEffect[j].name ) {
                ...
                case 'imageScale150Outside':
                    jQuery(g_baseControl).css('overflow','visible');
                    $g_containerThumbnails.css('overflow','visible');
                    jQuery(elt).css('overflow','visible');
                    jQuery(elt).find('.subcontainer').css('overflow','visible');
                    setElementOnTop(jQuery(elt).find('.imgContainer'), jQuery(elt).find('.labelImage'));
                    break;
                ...
                case 'rotateCornerBL':
                    ...
                    var e=jQuery(elt).find('.labelImage')[0];
                    e.style[g_transformProperty+'Origin'] = '100% 100%';
                    e.style[g_transformProperty] = (g_aengine=='velocity'?'rotateZ':'rotate')+'(-90deg)';
                    e=jQuery(elt).find('.imgContainer')[0];
                    e.style[g_transformProperty+'Origin'] = '100% 100%';
                    e.style[g_transformProperty] = (g_aengine=='velocity'?'rotateZ':'rotate')+'(0deg)';
                    break;
                ...
                case 'rotateCornerBR':
                    ...
                    jQuery(elt).find('.labelImage').css('opacity','1');
                    var e=jQuery(elt).find('.labelImage')[0];
                    e.style[g_transformProperty+'Origin'] = '0% 100%';
                    e.style[g_transformProperty] = (g_aengine=='velocity'?'rotateZ':'rotate')+'(90deg)';
                    e=jQuery(elt).find('.imgContainer')[0];
                    e.style[g_transformProperty+'Origin'] = '0 100%';
                    e.style[g_transformProperty] = (g_aengine=='velocity'?'rotateZ':'rotate')+'(0deg)';
                ...
                case 'imageRotateCornerBL':
                    ...
                    jQuery(elt).css('overflow','hidden');
                    jQuery(elt).find('.labelImage').css('opacity','1');
                    var e=jQuery(elt).find('.imgContainer')[0];
                    e.style[g_transformProperty+'Origin'] = 'bottom right';
                    e.style[g_transformProperty] = (g_aengine=='velocity'?'rotateZ':'rotate')+'(0)';
                    break;
                ...
                case 'slideUp':
                    jQuery(elt).css('overflow','hidden');
                    jQuery(elt).find('.labelImage').css({'opacity':'1', 'top':item.thumbRealHeight});
                    jQuery(elt).find('.imgContainer').css({'left':'0', 'top':'0'});
                    break;

                case 'slideDown':
                    jQuery(elt).css('overflow','hidden');
                    jQuery(elt).find('.labelImage').css({'opacity':'1', 'top':-item.thumbRealHeight, 'background':'none'});
                    jQuery(elt).find('.imgContainer').css({'left':'0', 'top':'0'});
                    break;

                case 'slideRight':
                    jQuery(elt).css('overflow','hidden');
                    jQuery(elt).find('.labelImage').css({'opacity':'1', 'left':-item.thumbRealWidth});
                    jQuery(elt).find('.imgContainer').css({'left':'0', 'top':'0'});
                    break;
                ...
                case 'labelSlideDown':
                    jQuery(elt).css('overflow','hidden');
                    jQuery(elt).find('.labelImage').css({'top':-item.thumbRealHeight, 'bottom':''});
                    break;
    ...
    function ThumbnailHover( elt ) {
        ...
        try {
            for( j=0; j<g_thumbnailHoverEffect.length; j++) {
                switch(g_thumbnailHoverEffect[j].name ) {
                    case 'imageSplit4':
                        jQuery(elt).find('.imgContainer').eq(0)[g_aengine]({'right':'50%', 'top':'-50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.imgContainer').eq(1)[g_aengine]({'left':'50%', 'top':'-50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.imgContainer').eq(2)[g_aengine]({'left':'50%', 'bottom':'-50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.imgContainer').eq(3)[g_aengine]({'right':'50%', 'bottom':'-50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'imageSplitVert':
                        jQuery(elt).find('.imgContainer').eq(0)[g_aengine]({'right':'50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.imgContainer').eq(1)[g_aengine]({'left':'50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelSplit4':
                        jQuery(elt).find('.labelImage').eq(0)[g_aengine]({'right':'50%', 'top':'-50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(1)[g_aengine]({'left':'50%', 'top':'-50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(2)[g_aengine]({'left':'50%', 'top':'50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(3)[g_aengine]({'right':'50%', 'top':'50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelSplitVert':
                        jQuery(elt).find('.labelImage').eq(0)[g_aengine]({'right':'50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(1)[g_aengine]({'left':'50%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelAppearSplit4':
                        jQuery(elt).find('.labelImage').eq(0)[g_aengine]({'right':'0%', 'top':'0%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(1)[g_aengine]({'left':'0%', 'top':'0%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(2)[g_aengine]({'left':'0%', 'top':'0%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(3)[g_aengine]({'right':'0%', 'top':'0%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelAppearSplitVert':
                        jQuery(elt).find('.labelImage').eq(0)[g_aengine]({'right':'0%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage').eq(1)[g_aengine]({'left':'0%'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'scaleLabelOverImage':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'scale':'0.5'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'overScale':
                    case 'overScaleOutside':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'opacity': '0', 'scale':'0.5'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'imageInvisible':
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'opacity': '0'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'rotateCornerBL':
                        var r=(g_aengine=='transition'?{rotate:'0deg'}:{rotateZ:'0deg'});
                        jQuery(elt).find('.labelImage')[g_aengine](r, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        r=(g_aengine=='transition'?{rotate:'90deg'}:{rotateZ:'90deg'});
                        jQuery(elt).find('.imgContainer')[g_aengine](r, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'slideUp':
                        ...
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'top': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'top': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'slideRight':
                        ...
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'left': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'left': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'labelAppear':
                        if( g_aengine == 'velocity' ) {
                            jQuery(elt)[g_aengine]({ 'backgroundColorRed':g_custGlobals.oldLabelRed, 'backgroundColorGreen':g_custGlobals.oldLabelGreen, 'backgroundColorBlue':g_custGlobals.oldLabelBlue, 'backgroundColorAlpha':1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        else {
                            var c='rgba('+g_custGlobals.oldLabelRed+','+g_custGlobals.oldLabelGreen+','+g_custGlobals.oldLabelBlue+',1)';
                            jQuery(elt).find('.labelImage')[g_aengine]({ 'backgroundColor': c}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        }
                        jQuery(elt).find('.labelImageTitle')[g_aengine]({ 'opacity': '1'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelFolderTitle')[g_aengine]({ 'opacity': '1'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelDescription')[g_aengine]({ 'opacity': '1'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelAppear75':
                        if( g_aengine == 'velocity' ) {
                            jQuery(elt).find('.labelImage')[g_aengine]({ 'backgroundColorRed':g_custGlobals.oldLabelRed, 'backgroundColorGreen':g_custGlobals.oldLabelGreen, 'backgroundColorBlue':g_custGlobals.oldLabelBlue, 'backgroundColorAlpha':0.75 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        else {
                            var c='rgba('+g_custGlobals.oldLabelRed+','+g_custGlobals.oldLabelGreen+','+g_custGlobals.oldLabelBlue+',0.75)';
                            jQuery(elt).find('.labelImage')[g_aengine]({'backgroundColor': c},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        jQuery(elt).find('.labelImageTitle')[g_aengine]({ 'opacity': '1'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelFolderTitle')[g_aengine]({ 'opacity': '1'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelDescription')[g_aengine]({ 'opacity': '1'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                    ...
                    case 'labelSlideUp':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'bottom': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'labelOpacity50':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'opacity': 0.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'imageOpacity50':
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'opacity': 0.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'borderLighter':
                        if( g_aengine == 'velocity' ) {
                            ...
                            jQuery(elt)[g_aengine]({ 'borderColorRed':co[0], 'borderColorGreen':co[1], 'borderColorBlue':co[2], 'colorAlpha':co[3] },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        }
                        else {
                            jQuery(elt)[g_aengine]({ 'borderColor': lighterColor(g_custGlobals.oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        }
                        break;
                    ...
                    case 'borderDarker':
                        if( g_aengine == 'velocity' ) {
                            ...
                            jQuery(elt)[g_aengine]({ 'borderColor': darkerColor(g_custGlobals.oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        }
                        break;

                    case 'imageScale150':
                        jQuery(elt).find('img')[g_aengine]({ scale: 1.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'scale120':
                        setElementOnTop('', elt);
                        //jQuery(elt).css('z-index','auto');
                        jQuery(elt)[g_aengine]({ scale: 1.2 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'imageFlipHorizontal':
                        setElementOnTop('', elt);
                        var n= Math.round(item.thumbRealHeight*1.2) + 'px';
                        jQuery(elt).find('.imgContainer')[g_aengine]({ perspective: n, rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ perspective: n, rotateX: '360deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'imageFlipVertical':
                        setElementOnTop('', elt);
                        var n= Math.round(item.thumbRealWidth*1.2) + 'px';
                        jQuery(elt).find('.imgContainer')[g_aengine]({ perspective: n, rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ perspective: n, rotateY: '360deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'flipHorizontal':
                        setElementOnTop('', elt);
                        var n= Math.round(item.thumbRealHeight*1.2) + 'px';
                        jQuery(elt)[g_aengine]({ perspective: n, rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'flipVertical':
                        setElementOnTop('', elt);
                        var n= Math.round(item.thumbRealWidth*1.2) + 'px';
                        jQuery(elt)[g_aengine]({ perspective: n, rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'TEST':
                        //jQuery(elt).find('img').stop(true, true);
                        jQuery(elt).find('.subcontainer')[g_aengine]({ scale: 0.8 },150).transition({ perspective: '50px', rotateX: '180deg'}, 300, 'ease').transition({ scale: 1 },150);
                        break;
                }
            }
        }
    ...
    function ThumbnailHoverOut( elt ) {
        ...
        try {
            for( j=0; j<g_thumbnailHoverEffect.length; j++) {
                switch(g_thumbnailHoverEffect[j].name ) {
                    case 'imageSplit4':
                        jQuery(elt).find('.imgContainer').eq(0)[g_aengine]({'right':'0%', 'top':'0%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer').eq(1)[g_aengine]({'left':'0%', 'top':'0%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer').eq(2)[g_aengine]({'left':'0%', 'bottom':'0%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer').eq(3)[g_aengine]({'right':'0%', 'bottom':'0%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'imageSplitVert':
                        jQuery(elt).find('.imgContainer').eq(0)[g_aengine]({'right':'0%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer').eq(1)[g_aengine]({'left':'0%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;
                    ...
                    case 'labelAppearSplitVert':
                        jQuery(elt).find('.labelImage').eq(0)[g_aengine]({'right':'50%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.labelImage').eq(1)[g_aengine]({'left':'50%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'scaleLabelOverImage':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'opacity': '0', 'scale':'0.5'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'scale':'1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'overScale':
                    case 'overScaleOutside':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'opacity': '0', 'scale':'1.5'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'imageInvisible':
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'opacity': '1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;
                    ...
                    case 'imageRotateCornerBR':
                        var r=(g_aengine=='transition'?{rotate:'0deg'}:{rotateZ:'0deg'});
                        jQuery(elt).find('.imgContainer')[g_aengine](r ,g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack );
                        break;

                    case 'slideUp':
                        var n= item.thumbRealHeight + 'px';
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'top': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'top': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'slideRight':
                        var n= '-'+item.thumbRealWidth + 'px';
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'left': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'left': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'imageSlideLeft':
                    case 'imageSlideRight':
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'top': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'left': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'labelAppear':
                    case 'labelAppear75':
                        if( g_aengine == 'velocity' ) {
                            jQuery(elt).find('.labelImage')[g_aengine]({ 'backgroundColorRed':g_custGlobals.oldLabelRed, 'backgroundColorGreen':g_custGlobals.oldLabelGreen, 'backgroundColorBlue':g_custGlobals.oldLabelBlue, 'backgroundColorAlpha':0 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        else {
                            var c='rgb('+g_custGlobals.oldLabelRed+','+g_custGlobals.oldLabelGreen+','+g_custGlobals.oldLabelBlue+',0)';
                            jQuery(elt).find('.labelImage')[g_aengine]({ 'backgroundColor': c},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        jQuery(elt).find('.labelImageTitle')[g_aengine]({ 'opacity': '0'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelFolderTitle')[g_aengine]({ 'opacity': '0'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelDescription')[g_aengine]({ 'opacity': '0'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelSlideDown':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'top': '-99%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'labelSlideUpTop':
                        var p= item.thumbRealHeight + 'px';
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'top': p},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'labelSlideUp':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'bottom': '-99%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;
                    ...
                    case 'labelOpacity50':
                        jQuery(elt).find('.labelImage')[g_aengine]({ 'opacity': g_custGlobals.oldLabelOpacity },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'imageOpacity50':
                        jQuery(elt).find('.imgContainer')[g_aengine]({ 'opacity': 1 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'borderDarker':
                        if( g_aengine == 'velocity' ) {
                            ...
                            var co = colorString.substring(colorString.indexOf('(') + 1, colorString.lastIndexOf(')')).split(/,\s*/);
                            jQuery(elt)[g_aengine]({ 'borderColorRed':co[0], 'borderColorGreen':co[1], 'borderColorBlue':co[2], 'colorAlpha':co[3] },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        else {
                            jQuery(elt)[g_aengine]({ 'borderColor': g_custGlobals.oldBorderColor },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        }
                        break;

                    case 'imageScale150':
                    case 'imageScale150Outside':
                        jQuery(elt).find('img')[g_aengine]({ scale: 1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'scale120':
                        jQuery(elt)[g_aengine]({ scale: 1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;
                    ...
                    case 'imageFlipHorizontal':
                        ...
                        jQuery(elt).find('.imgContainer')[g_aengine]({ perspective: n, rotateX: '0deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ perspective: n, rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;
                    ...
                    case 'imageFlipVertical':
                        ...
                        jQuery(elt).find('.imgContainer')[g_aengine]({ perspective: n, rotateY: '0deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        jQuery(elt).find('.labelImage')[g_aengine]({ perspective: n, rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
                        break;

                    case 'flipHorizontal':
                        var n= Math.round(g_oneThumbnailHeight*1.2) + 'px';
                        jQuery(elt)[g_aengine]({ perspective:n, rotateX: '0deg'}, g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;

                    case 'flipVertical':
                        var n= Math.round(item.thumbRealWidth*1.2) + 'px';
                        jQuery(elt)[g_aengine]({ perspective:n, rotateY: '0deg'}, g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
                        break;
    ...
    function OpenInternalViewer( imageIdx ) {
        ...
        //sImg+='<img class="image" src="//:0" alt="" style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
        //sImg+='<img class="image" src="//:0" alt="" style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
        sImg+='<img class="image" src="'+g_ngItems[imageIdx].responsiveURL()+'" alt="" style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
        sImg+='<img class="image" src="'+g_ngItems[imageIdx].responsiveURL()+'" alt="" style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
        sImg+='<img class="image" src="'+g_ngItems[imageIdx].responsiveURL()+'" alt="" style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
        ...
        $g_containerViewerToolbar=jQuery('<div class="toolbarContainer" style="visibility:hidden;"><div class="toolbar"><div class="previousButton ngbt"></div><div class="pageCounter"></div><div class="nextButton ngbt"></div><div class="playButton playPauseButton ngbt"></div>'+fs+sInfo+'<div class="closeButton ngbt"></div><div class="label"><div class="title" itemprop="name"></div><div class="description" itemprop="description"></div></div></div>').appendTo($g_containerViewer);
...
// ATTENTION --> 2 FOIS
        ...
        $g_containerViewerToolbar.find('.infoButton').on("click",function(e){
            e.stopPropagation();
            if( typeof gO.fnViewerInfo == 'function' ) {
                gO.fnViewerInfo(g_ngItems[g_viewerCurrentItemIdx]);
            }
        });
...
//        ResizeInternalViewer($g_ViewerImageNext);
                ...
                if( g_aengine == 'velocity' ) {
                    $g_ViewerImageCurrent.velocity(animImgCurrent, 500),
                        $g_ViewerImageNext.velocity({'opacity': 1 }, 300)
                    setTimeout(function () {
...
//        ResizeInternalViewer($g_ViewerImagePrevious);
                ...
                if( g_aengine == 'velocity' ) {
                    $g_ViewerImageCurrent.velocity(animImgCurrent, 500),
                        $g_ViewerImagePrevious.velocity({'opacity': 1 }, 300)
                    setTimeout(function () {
                        DisplayInternalViewerComplete(imageIdx, displayType);
                    }, 500);
                }
            ...
            default:
                $g_containerViewerContent.find('*').stop(true,true);
                $g_ViewerImageCurrent.css({'opacity':0, 'left':'0', visibility: 'visible'}).attr('src','data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=').attr('src',g_ngItems[imageIdx].responsiveURL());
    ...
    function DisplayInternalViewerComplete( imageIdx, displayType ) {
        ...
        $g_ViewerImageNext.css({'opacity':0, 'left':'0', visibility: 'hidden'}).attr('src','data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=').attr('src',g_ngItems[GetNextImageIdx(imageIdx)].responsiveURL());
        ...
        $g_ViewerImagePrevious.css({'opacity':0, 'left':'0', visibility: 'hidden'}).attr('src','data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=').attr('src',g_ngItems[GetPreviousImageIdx(imageIdx)].responsiveURL());
...
//    $g_containerViewerToolbar.css({'visibility':'visible'});
    ...
    function ResizeInternalViewer($imgElt) {
        ...
        window.clearInterval(g_viewerResizeTimerID);
        g_viewerResizeTimerID=window.setInterval(function(){ResizeInternalViewer($g_ViewerImageCurrent)},200);

        //var windowsW=jQuery(window).width();
        var windowsW=$g_containerViewer.width();
        //var windowsH=jQuery(window).height();
        var windowsH=$g_containerViewer.height();
        ...
        var imgPaddingH=Math.abs($elt.innerWidth()-$elt.width());
        ...
        $g_containerViewerContent.children('img').css({'max-width':(w-tH), 'max-height':(h-tV) });
    }
